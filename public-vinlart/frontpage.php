    ...
    <script>
        ...
        <div class="cok modal fade show" id="create_modal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-modal="true" role="dialog" style="display: block;  background-color: #ffffff3b;">
            <div class="modal-dialog snok modal-dialog-centered modal-dialog-scrollable">
                <div style="padding-top:27px;" class="tows cok modal-content">
...
function goBack() {
  ...
  <path fill-rule="evenodd" d="M15 8a.5.5 0 0 0-.5-.5H2.707l3.147-3.146a.5.5 0 1 0-.708-.708l-4 4a.5.5 0 0 0 0 .708l4 4a.5.5 0 0 0 .708-.708L2.707 8.5H14.5A.5.5 0 0 0 15 8z"></path>
</svg> <span data-key="banner"></span></span>
                    ...
                    <div style="overflow-y: scroll; height: 200px;" class="modal-body">
...
// Ruta de la imagen actual del usuario
$ruta_imagen = isset($usuario['banner']) ? 'banner/' . $usuario['banner'] : '';

// Procesar la carga o edición de imagen si se envió un formulario
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  if (isset($_FILES['imagen'])) {
    $imagen = $_FILES['imagen'];
    $nombre_imagen = $imagen['name'];
    $ruta_temporal = $imagen['tmp_name'];
    $ruta_destino = 'banner/' . $nombre_imagen;
    
    // Mover la imagen a la carpeta de destino
    if (move_uploaded_file($ruta_temporal, $ruta_destino)) {
      // Actualizar el banner del usuario
      $usuario['banner'] = $nombre_imagen;
...
function redirigir() {
  setTimeout(function() {
    window.location.href = "profile.php?user=<?php echo $d; ?>";
  }, 1000); // 3000 milisegundos = 3 segundos
}

// Llama a la función para iniciar el redireccionamiento
redirigir();
  </script>';
      exit();
    } else {
      echo 'No subiste nada';
      echo '<script>
      function redirigir() {
  setTimeout(function() {
    window.history.back();
...
exit();
  ...
  } elseif (isset($_POST['editar_banner'])) {
    // Eliminar el banner actual
    unset($usuario['banner']);
    // Guardar los cambios en el archivo JSON
    $usuarios[$nombre] = $usuario;
    file_put_contents('usuarios.json', json_encode($usuarios));
 ...
 <style>
  ...
  <!-- Agrega la referencia a la librería Croppie -->
   <link href="https://cdnjs.cloudflare.com/ajax/libs/croppie/2.6.5/croppie.min.css" rel="stylesheet">
                        <div style="height:100%; background-size: cover; background-repeat: no-repeat; background-position: center center;" id="image-container" class="hide_img cropped-image-container">
                        </div>
